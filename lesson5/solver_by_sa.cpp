...
class TSP {
 public:
  ...
  // Optimize a tour by simulated annealing (SA) algorithm.
  void OptimizeBySA(double start_temp, double end_temp, long long time_limit) {
    // Prepare a random number generator.
    std::mt19937 mt(1728);
    std::uniform_int_distribution<> rand_for_idx(1, N_ - 2);

    tmp_tour_ = current_best_tour_;
    double score = GetShortestLength();
    double best_score = GetShortestLength();
    long long time = 0;

    while (time != time_limit) {
      // Generate a random index in tour vector.
      int idx = rand_for_idx(mt);
      // Calculate the diff when tour[idx] and tour[idx + 1] are swapped
      double diff = GetDiffWhenAdjacentPointsSwapped(idx);
      // std::cout << diff << std::endl;
      double temp = start_temp + (end_temp - start_temp) * time / time_limit;
      // Calculate the probability that this modification is accepted.
      // If diff < 0 (when score gets smaller), prob > 1.
      double prob = exp((-1 * diff) / temp);
      double random_prob = (rand() % (int)inf) / (double)inf;
      if (prob > random_prob || diff < 0) {  // If this modification is accepted
        // Update vector and score.
        double tmp = tmp_tour_[idx];
        tmp_tour_[idx] = tmp_tour_[idx + 1];
        tmp_tour_[idx + 1] = tmp;
        score += diff;
        if (score < best_score) {
          best_score = score;
 ...
 private:
  ...
  // Get the difference of the score when two adjacent node in tour vector are
  // swapped.
  // Return [score of the modified vector] - [score of the original vector].
  double GetDiffWhenAdjacentPointsSwapped(int idx) {
    return (dist_[tmp_tour_[idx - 1]][tmp_tour_[idx + 1]] +
            dist_[tmp_tour_[idx + 2]][tmp_tour_[idx]]) -
           (dist_[tmp_tour_[idx - 1]][tmp_tour_[idx]] +
            dist_[tmp_tour_[idx + 1]][tmp_tour_[idx + 2]]);
  }
...
int main(int argc, char* argv[]) {
  ...
  if (a.exist("output")) {
    ...
    output_file_path =
        "./output/" + GetOutputFileName(input_file_path, "2opt_sa");
  }
  ...
  // Finally, the tour vector is optimized by SA algorithm.
  tsp.OptimizeBySA(100, 10, 1000000000);
  double shortest_length_after_sa = tsp.GetShortestLength();
  std::vector<int> shortest_tour_after_sa = tsp.GetShortestTour();
  std::cout << shortest_length_after_sa << std::endl;

  OutputTourToFile(shortest_tour_after_sa, output_file_path);
}
